...
private[format] object Parser extends RegexParsers{
  def apply(text: String): Either[String, Json.Value] =
  ...
  def members:  Parser[Map[String, Json.Value]] = rep1sep(member , ",") ^^ (_.toMap)  | (ws ^^ (_=> Map.empty ))
  def member: Parser[(String, Json.Value)]
    = (ws ~> string <~ ws) ~ (":" ~> element) ^^ {case string ~ element => string.value -> element}
